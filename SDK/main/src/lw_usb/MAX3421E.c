...
void MAXreg_wr(BYTE reg, BYTE val) {
	u8 SenderBuf[2] = {reg | 2, val};
	//psuedocode:
	//select MAX3421E
	XSpi_SetSlaveSelect(&SpiInstance,SpiInstance.SlaveSelectMask);
	//write reg + 2 via SPI
	//write val via SPI
	Status = XSpi_Transfer(&SpiInstance, SenderBuf,SenderBuf, 2);
	//read return code from SPI peripheral (see Xilinx examples)
	//if return code != 0 print an error
	if(Status !=  XST_SUCCESS){
			xil_printf ("Read SPI failed with status number %d.\n", Status);
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	Status = XSpi_Transfer(&SpiInstance, &Buf, &Buf, nbytes+1);
